...
const labelSizes: Record<InputFieldSize, string> = {
  sm: 'var(--body-sm)',
  md: 'var(--body-sm)',
  lg: 'var(--body-md)',
};
...
export function InputField({
  ...
  const borderColor = (() => {
    ...
    if (focused)   return 'var(--action-primary)';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', width: '100%' }}>
      ...
      <div
        style={{
          ...
          paddingLeft: leadingIcon ? 'var(--space-3)' : sz.px,
      ...
      >
        {leadingIcon && (
          <span style={{ color: hasError ? 'var(--status-error)' : 'var(--text-subtle)', flexShrink: 0, display: 'flex' }}>
            {leadingIcon}
          </span>
        )}
      ...
      {(helperText || errorText) && (
        <p
          ...
          style={{
            ...
            color: hasError ? 'var(--status-error)' : 'var(--text-subtle)',
            lineHeight: 1.4,
            margin: 0,
          }}
