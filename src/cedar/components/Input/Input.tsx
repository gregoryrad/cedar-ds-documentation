...
export function Input({
  ...
  return (
    ...
    >
      {leadingIcon && (
        <span style={{ color: disabled ? 'var(--text-disabled)' : hasError ? 'var(--status-error)' : 'var(--text-subtle)', flexShrink: 0, display: 'flex' }}>
          {leadingIcon}
        </span>
      )}
      ...
      {hasError && !trailingIcon && (
        <span aria-hidden="true" style={{ flexShrink: 0, color: 'var(--status-error)', display: 'flex' }}>
          <svg width={sz.iconSize} height={sz.iconSize} viewBox="0 0 16 16" fill="none">
