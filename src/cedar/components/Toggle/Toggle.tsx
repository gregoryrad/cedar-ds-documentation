...
function ToggleControl({
  ...
  id,
  labelledBy,
}: Pick<ToggleProps, 'checked' | 'onChange' | 'disabled' | 'size' | 'id'> & { labelledBy?: string }) {
  const sz = SIZES[size];
  const pad = 2;
  ...
  return (
    ...
    >
      <span
        ...
        style={{
          position: 'absolute',
          left: checked ? `${pad + travel}px` : `${pad}px`,
          width: sz.thumbSize,
          height: sz.thumbSize,
          borderRadius: 'var(--radius-full)',
          backgroundColor: 'var(--foundations-white)',
...
export function Toggle({ checked, onChange, disabled, size = 'md', label, description, id }: ToggleProps) {
  ...
  if (description) {
    // Full variant — label + description on the right, toggle on the right edge
    return (
      <div
        style={{
          display: 'flex',
