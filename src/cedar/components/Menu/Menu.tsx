...
export function Menu({ trigger, items, placement = 'bottom-end' }: MenuProps) {
  ...
  function handleItemClick(item: Extract<MenuItem, { type: 'item' }>) {
        ...
        >
          {items.map((item, i) => {
            ...
            return (
              ...
              >
                {item.icon && (
                  <span aria-hidden="true" style={{ flexShrink: 0, color: item.disabled ? 'var(--text-disabled)' : item.destructive ? 'var(--status-error)' : 'var(--text-subtle)' }}>
                    {item.icon}
                  </span>
                )}
