...
const labelFontSizes: Record<CheckboxSize, string> = {
  sm: 'var(--body-sm)',
  md: 'var(--body-md)',
  lg: 'var(--body-lg)',
};
...
export function Checkbox({
  ...
  const borderColor = (() => {
    if (isDisabled) return 'var(--border-disabled)';
    if (hasError)   return 'var(--status-error)';
    if (isFilled)   return 'var(--action-primary)';
    return 'var(--border-base)';
  })();

  const bgColor = (() => {
    if (isDisabled && isFilled) return 'var(--bg-disabled)';
    if (isDisabled)             return 'var(--bg-disabled)';
    if (isFilled)               return 'var(--action-primary)';
    return 'var(--bg-base)';
  })();
      ...
      >
        <span
          style={{
            ...
            fontSize: labelFontSizes[checkboxSize],
            color: isDisabled ? 'var(--text-disabled)' : 'var(--text-base)',
            lineHeight: 1.4,
          }}
        >
          {label}
