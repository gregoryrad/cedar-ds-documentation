...
const LABEL_SIZE: Record<RadioSize, string> = {
  sm: 'var(--body-sm)',
  md: 'var(--body-md)',
  lg: 'var(--body-lg)',
};
...
export function Radio({
    ...
    <span style={{ position: 'relative', flexShrink: 0, display: 'inline-flex' }}>
      ...
      <span
        ...
        style={{
          ...
          border: `var(--border-width-bold) solid ${borderColor}`,
        ...
        {checked && (
          <span
            style={{
              ...
              height: dotSize,
              borderRadius: 'var(--radius-full)',
              backgroundColor: isDisabled ? 'var(--text-disabled)' : 'var(--action-on-primary)',
            }}
          />
        )}
      </span>
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontFamily: 'var(--font-base)', fontSize: LABEL_SIZE[radioSize], color: isDisabled ? 'var(--text-disabled)' : 'var(--text-base)', lineHeight: 1.4 }}>
          {label}
        </span>
        {description && (
...
export function RadioGroup({
  ...
  return (
    <fieldset
      role="radiogroup"
      aria-labelledby={groupLabel ? legendId : undefined}
    ...
    >
      ...
      <div
        style={{
          ...
          gap: layout === 'horizontal' ? 'var(--space-5)' : 'var(--space-3)',
          flexWrap: 'wrap',
        }}
      >
        {options.map(opt => (
      ...
      {error && (
        <p style={{ margin: 'var(--space-2) 0 0', fontSize: 'var(--size-xs)', fontFamily: 'var(--font-base)', color: 'var(--status-error)' }}>
          {error}
        </p>
      )}
    </fieldset>
