...
export function Pagination({ totalPages, currentPage, onPageChange }: PaginationProps) {
  ...
  function NavButton({ dir }: { dir: 'prev' | 'next' }) {
    ...
    return (
      <button
        onClick={() => enabled && onPageChange(target)}
        disabled={!enabled}
        aria-label={dir === 'prev' ? 'Previous page' : 'Next page'}
        style={{
          ...BTN_BASE,
          color: enabled ? 'var(--text-subtle)' : 'var(--text-disabled)',
          ...
          item === 'ellipsis' ? (
            ...
            <li key={item}>
              <button
                ...
                style={{
                  ...BTN_BASE,
                  backgroundColor: item === currentPage ? 'var(--action-primary)' : 'transparent',
                  color:           item === currentPage ? 'var(--action-on-primary)' : 'var(--text-subtle)',
