...
export function Select({
  ...
  const borderColor = (() => {
    ...
    if (focused || open) return 'var(--action-primary)';
    return 'var(--border-subtle)';
  })();

  const borderWidth = (focused || open) && !disabled
  ...
  // Set initial focused idx when opening
  useEffect(() => {
    if (!open) return;
  ...
  function handleListboxKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === 'Escape') {
      ...
      e.preventDefault();
      setFocusedIdx(enabledIdxs[(pos + 1) % enabledIdxs.length]);
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setFocusedIdx(enabledIdxs[(pos - 1 + enabledIdxs.length) % enabledIdxs.length]);
    } else if (e.key === 'Home') {
      e.preventDefault();
      setFocusedIdx(enabledIdxs[0]);
    } else if (e.key === 'End') {
      e.preventDefault();
      setFocusedIdx(enabledIdxs[enabledIdxs.length - 1]);
  ...
  function handleOptionClick(opt: SelectOption) {
    ...
    <div ref={wrapperRef} style={{ position: 'relative', width: '100%' }}>
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: disabled ? 'var(--bg-disabled)' : 'var(--bg-base)',
          fontFamily: 'var(--font-base)',
          fontSize: sz.fontSize,
          color: selected
            ? (disabled ? 'var(--text-disabled)' : 'var(--text-base)')
            : 'var(--text-subtle)',
              ...
              <li
                ...
                style={{
                  ...
                  color: opt.disabled
                    ? 'var(--text-disabled)'
                    : 'var(--text-base)',
                  backgroundColor: isFocused && !opt.disabled ? 'var(--bg-faint)' : 'transparent',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', width: '100%' }}>
      <label
        ...
        style={{
          fontSize: 'var(--body-sm)',
          fontWeight: 500,
          color: selectProps.disabled ? 'var(--text-disabled)' : 'var(--text-base)',
          fontFamily: 'var(--font-base)',
        }}
      >
        {label}
